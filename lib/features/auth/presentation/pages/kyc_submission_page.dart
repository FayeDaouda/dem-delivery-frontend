...
class _KycSubmissionPageState extends State<KycSubmissionPage> {
  ...
  Future<void> _uploadDocument(String fieldKey) async {
    ...
    if (filePath.isEmpty) {
      AppDialog.showWarning(context,
          'Veuillez saisir le chemin du fichier (${_labels[fieldKey]}).');
      return;
    }
    ...
    try {
      ...
      final uploadUrl =
          (data['uploadUrl'] ?? data['data']?['uploadUrl'])?.toString();
      ...
      AppDialog.showSuccess(
          context, '${_labels[fieldKey]} uploadé avec succès.');
  ...
  Future<void> _submitKyc() async {
    ...
    final missing = _uploadedUrls.entries
        .where((entry) => entry.value == null || entry.value!.isEmpty)
        .toList();
    if (missing.isNotEmpty) {
      AppDialog.showWarning(
          context, 'Veuillez uploader les 6 documents avant soumission.');
      return;
    }
    ...
    try {
      final payload = <String, dynamic>{
        ...
        'vehicleRegistrationVersoUrl':
            _uploadedUrls['vehicleRegistrationVersoUrl'],
        'motorcyclePhotoUrl': _uploadedUrls['motorcyclePhotoUrl'],
        'selfieUrl': _uploadedUrls['selfieUrl'],
      };

      final response =
          await _dio.post('/drivers/documents/submit', data: payload);
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if (!mounted) return;
        AppDialog.showSuccess(
            context, 'Documents soumis. Vérification en cours.');
        Navigator.pop(context, true);
      } else {
        if (!mounted) return;
        AppDialog.showError(
            context, 'Soumission KYC échouée (${response.statusCode}).');
      }
    } on DioException catch (e) {
      if (!mounted) return;
